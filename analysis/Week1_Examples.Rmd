...
```{r constants}
DATADIR <- "data/"
INPUTDATA <- paste0(DATADIR, "LungCapDAtaCSV.csv")
```

```{r loadData}
lungCap <- read.csv(INPUTDATA)
```
...
![Estimate our shading from the Standard Normal Distribution](Week1_ExamplesImages/standard-normal-distribution.png)
...
> Problem 6.1.45 (Bluman, pg 323)
>  Find the z-value for given area (aka p-value)
>  P(X > z) = 0.0239

```{r bluman6-1-45_approxgraph}
curve(dnorm(x), from=-4, to=4, 
